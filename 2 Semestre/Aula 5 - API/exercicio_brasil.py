...
def busca_cep(cep):
    dic_endereco = requests.get(f'https://viacep.com.br/ws/{cep}/json/').json()
    return dic_endereco
...
quantas_chaves_no_cep = len(busca_cep(cep_da_mirtes))
...
def cidade_do_cep(cep):
    return busca_cep(cep)['localidade']
...
def endereco_curto(endereco):
    return f'{endereco['logradouro']}, {endereco['bairro']} - {endereco['localidade']}/{endereco['uf']}'
...
def tem_complemento(endereco):
    return 'complemento' in endereco.keys() and endereco['complemento'] != ''
...
def achou_endereco(resposta):
    return 'erro' not in resposta.keys()
...
def status_do_cep(cep):
    dic_endereco = requests.get(f'https://viacep.com.br/ws/{cep}/json').status_code()
    return dic_endereco
...
'''
por_que_o_json_estourou = 'a'   # 'a', 'b', 'c' ou 'd'
