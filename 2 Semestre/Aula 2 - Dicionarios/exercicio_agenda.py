...
quantas_pessoas = len(agenda_exemplo.keys())
tem_fabio = 'fabio' in agenda_exemplo.keys()
tem_joao = 'joao' in agenda_exemplo.keys()
...
def conta_letras(palavra):
    contador = {}
    for letra in palavra:   # o for percorre a string letra por letra
        if letra not in contador.keys():
            contador[letra] = 1
        else:
            contador[letra] = contador[letra] + 1
    return contador# COMPLETE: se a letra ainda nao esta em contador, comece em 0; depois some 1 nela
    # COMPLETE: retorne o contador
...
'''
quantas_mirtes_previsto = len(agenda_mirtes.keys())
tem_bruno_na_cicero_previsto = 'bruno' in agenda_cicero.keys()
...
def main():
    ...
    while True:
        ...
        if opcao == "1":
            agenda = input('Digite o nome da Agenda: ')
            pessoa = input('Digite o nome da Pessoa: ')
            telefone = input('Digite o número do telefone: ')
            adiciona(agenda, pessoa, telefone)  # COMPLETE: monte a funcionalidade 1 com as pecas acima

        elif opcao == "2":
            # FUNCIONALIDADE 2 - consultar o telefone de uma pessoa na agenda da
            # Mirtes. Cheque ANTES com esta_na_agenda: SE a pessoa estiver, mostre
            # o telefone (consulta); SENAO, avise que ela nao esta. Monte o
            # if/else com as pecas abaixo (uma vira a condicao do if; um print vai
            # no if, o outro no else):
            #
            #     pessoa = input("  Nome: ")
            #     esta_na_agenda(agenda_mirtes, pessoa)
            #     print(f"  Telefone de {pessoa}: {consulta(agenda_mirtes, pessoa)}")
            #     print(f"  {pessoa} nao esta na agenda da Mirtes")
            #
            pessoa = input("Nome da Pessoa: ")  # COMPLETE: monte a funcionalidade 2 com as pecas acima
            print("Telefone de contato: ", consulta(agenda_mirtes, pessoa))

        elif opcao == "3":
            agenda = input('Digite o nome da Agenda: ')
            pessoa = input('Digite o nome da Pessoa: ')
            telefone = input('Digite o número do telefone: ')
            adiciona(agenda, pessoa, telefone)
        elif opcao == "4":
            pessoa = input("Nome da Pessoa: ")  # COMPLETE: monte a funcionalidade 2 com as pecas acima
            print("Telefone de contato: ", consulta(agenda_mirtes, pessoa))
        elif opcao == "5":
...
'''
quantas_apos_remover = 'coloque o valor aqui'
fabio_apos_remover = 'coloque o valor aqui'
...
'''
tel_da_aninha = 'coloque o valor aqui'
ana_ainda_existe = 'coloque o valor aqui'
quantas_apos_renomear = 'coloque o valor aqui'
...
'''
def renomeia(agenda, antigo, novo):
    pass
